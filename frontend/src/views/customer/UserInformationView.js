...
const MyInfoView = () => {
    ...
    const getUserInformation = async () => {
        const response = await axios.get('http://43.203.11.194:8081/api/user/getUserByUserId',
            {
                headers: {
    ...
    const getBoard = async () => {
        const response = await axios.get('http://43.203.11.194:8081/api/board/getBoardByUserIdx',
            {
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': 'Bearer ' + window.localStorage.getItem("jwt_token"),
    ...
    const getRepairs = async () => {
        const response = await axios.get('http://43.203.11.194:8081/api/repair/getRepairStatusByUserId',
            {
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': 'Bearer ' + window.localStorage.getItem("jwt_token"),
    ...
    return (
        <>
            ...
            <SideBar content={window.localStorage.getItem("name")} selectedMenu={{"as" : false, "board" : false, "chat" : false, "faq" : false, 'my' : true}}/>
            <section id="main">
